...
export class AuthService {
  ...
  async verifyEmail(token: string) {
    ...
    if (verification.expiresAt < new Date()) {
      throw new BadRequestException('El enlace de verificación ha expirado. Solicita uno nuevo dentro de los 15 minutos.');
    }
  ...
  async generateTwoFactorSecret(email: string) {
    const secret = speakeasy.generateSecret({ name: `PetSpa (${email})` });
  ...
  async enableTwoFactor(userId: string, token: string) {
    ...
    if (!user?.twoFactorSecret) {
      throw new BadRequestException('Primero genera el secreto 2FA');
    }
  ...
  async signIn(
    ...
    ) */ throw new ForbiddenException('Cuenta bloqueada temporalmente por 15 minutos.');
    }
    ...
    if (user.twoFactorEnabled) {
      return { requires2FA: true, email: user.email } as any;
    }
